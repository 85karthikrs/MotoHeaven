...
$html='
	<span>Ordered From:</span><img src="images/logo/2logoMOTO.png" height="40">
	<div class="wishlist-table table-responsive">
   <table>
      <thead>
         <tr>
         	<th class="product-thumbnail">Sl.No.</th>
      ...
      <tbody>';
		...
		$address_res=mysqli_query($con,"select address,pincode from `order` where id=$order_id ");
		$address_row=mysqli_fetch_assoc($address_res);
		$address_result=$address_row['address'];
		$pincode_result=$address_row['pincode'];

		while($row=mysqli_fetch_assoc($res)){
			$ordered_on=$row['added_on'];

			
		$total_price=$total_price+($row['qty']*$row['price']);
		 $pp=$row['qty']*$row['price'];
         $html.='<tr>
         	<td class="product-name">'.$serial.'</td>
            <td class="product-name">'.$row['name'].'</td>
            <td class="product-name"> <img height=200 src="'.PRODUCT_IMAGE_SITE_PATH.$row['image'].'"></td>
...
</div><p>Order ID: '.$order_id.' </p><p>Ordered on: '.$ordered_on.' </p> <p>Address: '.$address_result.' </p><p>Pincode: '.$pincode_result.' </p>';
