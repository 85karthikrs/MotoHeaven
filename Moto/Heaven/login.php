...
<!-- Start Bradcaump area -->
								...
								<form id="login-form" method="post">
									<div class="single-contact-form">
										<div class="contact-box name">
											<input type="text" name="login_email" id="login_email" placeholder="Your Email*" style="width:100%"  autocomplete="off">
										</div>
						...
						<div class="contact-form-wrap mt--60">
							...
							<div class="col-xs-12">
								<form id="register-form" method="post">
									...
									<div class="single-contact-form">
										<div class="contact-box name">
											<input type="text" name="email" id="email" autocomplete="off" placeholder="Your Email*" style="width:45%">
									...
									<div class="single-contact-form">
										<div class="contact-box name">
											<input type="text" autocomplete="off" name="mobile" id="mobile" pattern="^\d{10}$" title="10 numeric characters only" placeholder="Your Mobile*" style="width:45%">
		...
		function email_sent_otp(){
			...
			if(email!=''){
				if(email.match(mailformat)){
					...
					jQuery('#email_error').html('Wrong email id format');
				}
			}else{
			...
			if(mailTest=='verified'){
			if(email==''){
				jQuery('#email_error').html('Please enter email id');
			}else{
				jQuery('.email_sent_otp').html('Please wait..');
				//please wait button cant't be allowed to get clicked since the button has a function of sending otp..if the user clicks it unknowingly again and again..the function will be revoked again and again .. 
				...
				jQuery.ajax({
					...
					success:function(result){
						if(result=='done'){
							//to ensure user does not change email after verifying otp
		...
		function mobile_sent_otp(){
			...
			//check whether the entered input is numeric or not
			
			if(mobile!=""){
				if(isNaN(mobile)){
					jQuery('#mobile_error').html('Please enter 10 DIGITS only');
				}else{
			...
			if(phoneTest=='verified'){
			if(mobile==''){
				jQuery('#mobile_error').html('Please enter mobile number');
			}else if(mobile.length!=10){
				jQuery('#mobile_error').html('Please enter 10 digits');
			}else{
